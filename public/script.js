document.getElementById('city-form').addEventListener('submit', async (event) => {
  ...
  resultsDiv.innerHTML = `
    ...
    <h3>Meals</h3>
    <ul>
      ${result.meals.map(item => `<li>${item.item_name}: ${item.average_price} ${item.currency}</li>`).join('')}
    </ul>
    <h3>Transportation</h3>
    <ul>
      ${result.transportation.map(item => `<li>${item.item_name}: ${item.average_price} ${item.currency}</li>`).join('')}
    </ul>
  `;
});